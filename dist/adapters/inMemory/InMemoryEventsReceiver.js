"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.InMemoryEventsReceiver = void 0;
const node_events_1 = require("node:events");
const inversify_1 = require("inversify");
const EventHandlerRegistry_1 = require("../registry/EventHandlerRegistry");
let InMemoryEventsReceiver = class InMemoryEventsReceiver {
    eventEmitter;
    constructor(eventEmitter) {
        this.eventEmitter = eventEmitter;
    }
    async init() {
        const eventNames = EventHandlerRegistry_1.EventHandlerRegistry.getAllEventNames();
        for (const eventName of eventNames) {
            this.eventEmitter.on(eventName, (domainEvent) => {
                const eventHandlers = EventHandlerRegistry_1.EventHandlerRegistry.getEventHandler(eventName);
                return Promise.allSettled(eventHandlers.map(elem => elem.handle(domainEvent)));
            });
        }
    }
};
InMemoryEventsReceiver = __decorate([
    (0, inversify_1.injectable)(),
    __metadata("design:paramtypes", [node_events_1.EventEmitter])
], InMemoryEventsReceiver);
exports.InMemoryEventsReceiver = InMemoryEventsReceiver;
